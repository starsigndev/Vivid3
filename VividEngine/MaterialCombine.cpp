...
void MaterialCombine::Create() {

    //.\n");
    SetVertexShader("pp_combine.vsh");
    SetPixelShader("pp_combine.psh");
    ...
    v_sampler.Desc = v_rsampler;
    v_sampler.SamplerOrTextureName = "v_Texture2";
    v_sampler.ShaderStages = SHADER_TYPE_PIXEL;
    ...
    rl_desc.NumVariables = 2;

    rl_desc.NumImmutableSamplers = 2;
...
void MaterialCombine::Bind(bool sp) {

    m_SRB->GetVariableByName(SHADER_TYPE_PIXEL, "v_Texture")->Set(m_Diffuse->GetView(), SET_SHADER_RESOURCE_FLAG_NONE);
    m_SRB->GetVariableByName(SHADER_TYPE_PIXEL, "v_Texture2")->Set(m_Aux->GetView(), SET_SHADER_RESOURCE_FLAG_NONE);
...
//    auto light = Engine::m_Lights[0];
    ...
    map_data[0].v_Combine = float4(m_C1, m_C2,0, 0);
