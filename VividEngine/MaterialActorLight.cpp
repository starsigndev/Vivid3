...
void MaterialActorLight::Create() {
    ...
    SetVertexShader("mesh_actorlit.vsh");
    SetPixelShader("mesh_actorlit.psh");

    BasicUniform = CreateUniform(sizeof(ActorLightUniform), "Constants Actor Uniform Buffer - MVP");
    BasicUniform2 = CreateUniform(sizeof(ActorLightUniform), "Constants Actor Uniform Buffer - MVP");
...
void MaterialActorLight::Bind(bool sp) {
    ...
    if (sp) {
        ...
        MapHelper<ActorLightUniform> map_data(Engine::m_pImmediateContext, BasicUniform2, MAP_WRITE, MAP_FLAG_DISCARD);
        ...
        map_data[0].v_LightDiff = light->GetDiffuse();
        map_data[0].v_LightPos = float4(light->GetPosition(), 1.0f);
        map_data[0].v_LightSpec = float4(light->GetSpecular(), 1.0f);
        map_data[0].v_MatDiff = float4(1, 1, 1, 1);
        map_data[0].v_MatSpec = float4(1, 1, 1, 1);
        map_data[0].v_CameraPos = float4(Engine::m_Camera->GetPosition(), 1.0);
        map_data[0].v_LightProp = float4(light->GetRange(), 0, 0, 0);
        for (int i = 0; i < 100; i++) {
            map_data[0].bones[i] = float4x4::Identity().Transpose();
        }
    }
    else {
        ...
        map_data[0].v_LightProp = float4(light->GetRange(), 0, 0, 0);
        for (int i = 0; i < 100; i++) {
            map_data[0].bones[i] = m_Bones[i].Transpose();
